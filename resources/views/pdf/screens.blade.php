...
<body>
    ...
    <div class="title">Liste des prestations sur écrans</div>
            ...
            <tbody>
                @foreach ($provisions as $pe)
                    <tr style="vertical-align: start;">
                        ...
                        <td>{{ $pe->provision?->name }}</td>
                        <td style="font-size: 85%">
                            @if ($pe->provision?->dimensions_indicator)
                                {{ $pe->provision?->dimensions_indicator }}
                            @endif
                            @if ($pe->provision?->format_indicator)
                                · {{ $pe->provision?->format_indicator }}
                            @endif
                        </td>
                        <td>{{ $pe->media_status?->getLabel() }}</td>
                        <td>
