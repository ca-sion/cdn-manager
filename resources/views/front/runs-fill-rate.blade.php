...
<body class="min-h-full font-sans text-slate-800 {{ $isEmbed ? 'bg-transparent p-2 sm:p-4' : 'bg-gradient-to-b from-sky-50/60 via-slate-50 to-sky-50/40 py-8 px-4 sm:px-6 lg:px-8' }}">
    ...
    <div class="{{ $isEmbed ? 'w-full' : 'max-w-5xl mx-auto' }}">
        ...
        @if (! $isEmbed)
            <header class="mb-8 text-center sm:text-left sm:flex sm:items-center sm:justify-between border-b border-sky-100 pb-6">
                <div>
        ...
        {{-- Bandeau info indicatif & Légende --}}
        <div class="mb-6 p-3.5 rounded-xl bg-white/90 backdrop-blur-sm border border-sky-100 shadow-xs flex items-center justify-between flex-wrap gap-3">
            ...
            <div class="flex items-center gap-4 text-xs text-slate-500">
                <span class="inline-flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-sky-500"></span> Places dispo</span>
                <span class="inline-flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-amber-500"></span> &gt; 70%</span>
                <span class="inline-flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-rose-500"></span> Derniers dossards</span>
                <span class="inline-flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-slate-500"></span> Complet</span>
            </div>
        </div>

        {{-- Grille des courses épurée (sans boutons individuels) --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @forelse ($courses as $course)
                <div class="bg-white rounded-2xl p-5 border border-sky-100/80 shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between {{ $course['is_closed'] ? 'bg-slate-50/70 border-slate-200' : '' }}">
                    
                    {{-- Titre et Badge --}}
                    <div>
                        <div class="flex items-start justify-between gap-3 mb-2">
                            <h3 class="font-bold text-slate-900 text-base leading-snug {{ $course['is_closed'] ? 'text-slate-600' : '' }}">
                                {{ $course['name'] }}
                            </h3>

                            @php
                                $badgeClasses = match($course['color_theme']) {
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border shrink-0 {{ $badgeClasses }}">
                                {{ $course['badge_label'] }}
                            </span>
                        </div>

                        {{-- Jauge de progression (Bleu neige & dégradés discrets) --}}
                        <div class="my-3">
                            @if ($course['has_limit'] && $course['percentage'] !== null)
                                <div class="flex justify-between items-center text-xs font-semibold text-slate-600 mb-1.5">
                                    ...
                                    <span class="tabular-nums font-bold {{ $course['color_theme'] === 'slate' ? 'text-slate-700' : ($course['color_theme'] === 'rose' ? 'text-rose-600' : ($course['color_theme'] === 'amber' ? 'text-amber-600' : 'text-sky-700')) }}">
                                        {{ $course['percentage'] }}%
                                    </span>
                                </div>

                                <div class="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden p-0.5 border border-slate-200/70">
                                    @php
                            ...
                            @else
                                <div class="flex justify-between items-center text-xs font-semibold text-slate-600 mb-1.5">
                                    ...
                                    <span class="text-sky-600 font-medium">Ouverte</span>
                                </div>
                                <div class="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden p-0.5 border border-slate-200/70">
                        ...
                        @if (! empty($course['levels']))
                            <div class="mt-3.5 pt-3 border-t border-sky-100/70">
                                <div class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2 flex items-center justify-between">
                                    <span>Par degré scolaire</span>
                                    <span class="text-[10px] text-slate-400 normal-case">3H à 8H</span>
                                </div>
                                <div class="grid grid-cols-3 gap-2">
                                    @foreach ($course['levels'] as $lvl)
                                        <div class="bg-sky-50/50 rounded-xl p-2 border border-sky-100/80 flex flex-col justify-between">
                                            <div class="flex items-center justify-between text-xs mb-1">
                                                <span class="font-bold text-slate-800">{{ $lvl['level'] }}</span>
                                                <span class="tabular-nums font-semibold {{ $lvl['color_theme'] === 'slate' ? 'text-slate-600' : ($lvl['color_theme'] === 'rose' ? 'text-rose-600' : ($lvl['color_theme'] === 'amber' ? 'text-amber-600' : 'text-sky-700')) }}">
                                                    {{ $lvl['percentage'] }}%
                                                </span>
                                            </div>
                    ...
                    {{-- Pied de carte épuré : Statut & Date limite --}}
                    <div class="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 mt-2">
                        <span class="font-medium {{ $course['is_closed'] ? 'text-slate-400' : 'text-slate-600' }}">
                            {{ $course['status_label'] }}
                        </span>
        ...
        {{-- Pied de page --}}
        @if (! $isEmbed)
            <footer class="mt-12 pt-6 border-t border-sky-100 text-center text-xs text-slate-500">
