...
- [x] Task: Audit Existing Implementation
    - [x] Sub-task: Review `InvoiceController::generateQrBill` to confirm `StructuredAddress` usage.
    - [x] Sub-task: Verify `Client` model has necessary fields (street, house_number, etc.) and they are correctly used.
